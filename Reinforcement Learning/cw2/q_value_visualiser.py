...
class QValueVisualiser:
    ...
    def draw_q_values(self, q_network):
        #to accept a trained dqn
        # Create an empty image
        self.q_values_image.fill(0)
        # Loop over the grid cells and actions, and draw each q value
        for col in range(10):
            for row in range(10):
                #get state array of (x, y)
                state_array = np.array([((col/10.0) + 0.05), ((row/10.0) + 0.05)]).astype(np.float32)
                #pass the state input to dqn and get the predicted q values
                q_values = q_network.predict_single(state_array)
                ...
                for action in range(4):
                    ...
                    q_value_norm = (q_values[action] - min_q_value) / q_value_range
                    # Draw this q value
                    x = state_array[0]
                    y = state_array[1]
        ...
        # Show the image
        #cv2.imwrite('q_trained_values_image.png', self.q_values_image)
