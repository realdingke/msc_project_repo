...
class LinearLayer(Layer):
    ...
    def backward(self, grad_z):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        self._grad_W_current = self._cache_current.transpose() @ grad_z
        self._grad_b_current = np.ones((1, np.shape(grad_z)[0])) @ grad_z
...
            - neurons {list} -- Number of neurons in each linear layer 
        ...
        #######################################################################
        ACTIVATIONS_MAP = {
            "sigmoid": lambda size: SigmoidLayer(),
            "relu": lambda size: ReluLayer(),
            "identity": lambda size: LinearLayer(size, size)
        }

        self._layers = []
        prev_dim = input_dim
        for num_neurons, activation in zip(neurons, activations):
            self._layers.append(LinearLayer(prev_dim, num_neurons))
            self._layers.append(ACTIVATIONS_MAP[activation](num_neurons))
...
class Trainer(object):
    ...
    def __init__(
        ...
        self._loss_layer = LOSS_FUN_MAP[loss_fun]()
        #######################################################################
        #                       ** END OF YOUR CODE **
        #######################################################################
    ...
    def train(self, input_dataset, target_dataset):
        ...
        for i in range(self.nb_epoch):
            ...
            for input_batch, target_batch in zip(input_batches, target_batches):
                network_output = self.network(input_batch)
                self._loss_layer(network_output, target_batch)
                grad_loss_wrt_outputs = self._loss_layer.backward()
                self.network.backward(grad_loss_wrt_outputs)
...
class Preprocessor(object):
    ...
    def __init__(self, data):
        ...
        self.max_min_diff = np.max(data, axis=0) - self.min
    ...
    def apply(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        return (data - self.min) / self.max_min_diff
    ...
    def revert(self, data):
        ...
        #                       ** START OF YOUR CODE **
        #######################################################################
        return data * self.max_min_diff + self.min
